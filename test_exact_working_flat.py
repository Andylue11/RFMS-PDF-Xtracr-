...
def test_exact_working_flat_structure():
    """Test using the EXACT working flat payload structure that creates populated orders."""
    print("🎯 EXACT WORKING FLAT STRUCTURE TEST")
    print("=" * 60)
    ...
    print(f"   🔗 Endpoint: {BASE_URL}/order/create (NOT /v2/order/create)")  
    print(f"   📦 PO Number: {po_number}")
    print(f"   👤 Customer ID: 2 (from successful logs)")
    print(f"   📞 Supervisor: {supervisor_name} - {supervisor_phone}")
    
    # EXACT working payload structure (FLAT, not nested under "order")
    payload = {
        "category": "Order",
        "poNumber": po_number,
        "adSource": 1,
    ...
    print(f"📤 Sending request to: {BASE_URL}/order/create")
    ...
    if response.status_code == 200:
        try:
            ...
            if result.get("status") == "success":
                ...
                print(f"🎯 Used EXACT working flat structure!")
...
def main():
    """Run the exact working flat structure test."""
    success = test_exact_working_flat_structure()
    
    print(f"\n" + "=" * 60)
    print("🎯 FINAL RESULT:")
    if success:
        print("✅ SUCCESS! The exact working flat structure populated data!")
        print("🎉 We confirmed the correct payload format!")
    else:
        print("❌ Working flat structure still didn't show populated data")
        print("🔍 Data might be saved but not visible via this API endpoint")
        print("💡 Next: Check RFMS web interface to verify data persistence")
